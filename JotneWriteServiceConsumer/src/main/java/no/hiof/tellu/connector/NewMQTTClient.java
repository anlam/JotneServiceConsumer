...
public class NewMQTTClient implements MqttCallback, IMqttActionListener {
	...
	private LinkedBlockingQueue<MQTTMessageQueueElement> messageQueue = new LinkedBlockingQueue<MQTTMessageQueueElement>();
	private static Thread processMessageThread;
	...
	// public void setOrchestrationResult(OrchestrationResultDTO
	// orchestrationResult) {
	...
	public void processMessage() {
		while (true) {
			MQTTMessageQueueElement message = null;
			try {
				message = (MQTTMessageQueueElement) messageQueue.take();
			...
			if (message != null) {
				...
				if (sub_topic.equals("ruuvi_measurement")) {
					...
					sensorMeasurments
							.add(new SensorMeasurementDTO("latitude_err", String.valueOf(pl.getLatitude_err())));
					sensorMeasurments.add(new SensorMeasurementDTO("longitude", String.valueOf(pl.getLongitude())));
					sensorMeasurments
							.add(new SensorMeasurementDTO("longitude_err", String.valueOf(pl.getLongitude_err())));
					...
					sensorMeasurments
							.add(new SensorMeasurementDTO("altitude_err", String.valueOf(pl.getAltitude_err())));
	...
	public void messageArrived(String arg0, MqttMessage arg1) throws Exception {
		...
		messageQueue.add(new MQTTMessageQueueElement(topic, payload));
